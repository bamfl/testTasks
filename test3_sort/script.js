const sortByReviewsBtn = document.querySelector('.reviews');
...
const modifyArr = (arr) => {
	arr.forEach(item => {
		item.ratingRevievs = parseInt(item.ratingRevievs);
...
goodsList.classList.add('list');

const createGoodsList = (arr) => {
	goodsList.innerHTML = '';

	arr.forEach(({name, ratingRevievs, price}) => {
		goodsList.innerHTML += 
		`<h2>${name}</h2>
		<p>${ratingRevievs} отзыва(ов)</p>
		<p>${price} грн</p>
		`;
	});

	document.body.append(goodsList);
};

const sortByReviews = arr => {
	arr.sort((a, b) => a.ratingRevievs - b.ratingRevievs);
	createGoodsList(arr);
};
...
sortByReviewsBtn.addEventListener('click', () => sortByReviews(copyArr));
sortByPriceBtn.addEventListener('click', () => sortByPrice(copyArr));
